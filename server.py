...
class POP3Server(BaseRequestHandler): # 处理TCP连接
    ...
    ######### 处理"STAT"命令的部分，计算电子邮件数量以及总字节数，并返回给客户端：
                ...
                print("Response: +OK")
                num_emails = len(MAILBOXES[user])
                total_bytes =  sum(len(email.encode('utf-8')) for email in MAILBOXES[user])
                ...
                print("Response: +OK")
                response_list = []
                for index, email in enumerate(MAILBOXES[user], start=1):
                ...
                if email_idx >= len(MAILBOXES[user]):

                    response = "+OK\r\n not in the list\r\n"
                    conn.sendall(response.encode('utf-8') + b".\r\n")
                elif email_idx not in deleted_emails:

                    email_content = MAILBOXES[user][email_idx]
...
class SMTPServer(BaseRequestHandler):   #继承BaseRequestHandler类
        ...
        while True:
            try:
                    ...
                    if data == ".":
                        ...
                        for i in receiver:
                            ...
                            if receiver_.endswith("edu.cn") and args.name == "exmail.qq.com":
                                print("???")
                                MAILBOXES[receiver_].append(data)
                            elif receiver_.endswith("gmail.com") and args.name == "gmail.com":
                                MAILBOXES[receiver_].append(data)
                            elif receiver_ not in _config['accounts']["exmail.qq.com"].keys() and receiver_ not in _config['accounts']["gmail.com"].keys():
                                print("222")
                                sender_ = sender.strip("<>")
                                MAILBOXES[sender_].append(data)
                                print("444")
                ...
                if data.startswith("helo") or data.startswith("ehlo"):
                    ...
                    sender_ = sender.strip("<>")
                    if sender_ not in _config['accounts']["exmail.qq.com"].keys() and sender_ not in _config['accounts']["gmail.com"].keys():
                        print("www")
                        conn.sendall(b"550 \r\n")
                    else:
    ...
    def Sendmail(self,sender,receiver,msg):
        ...
        client_socket.connect(('localhost', int(fdns_query(TO_SERVER, 'P'))))

        client_socket.send(b'ehlo\r\n')
